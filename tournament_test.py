...
def testCount():
    ...
    if c != 0:
        raise ValueError("After deletion, countPlayers should return zero.%s" %c)
...
def testStandingsBeforeMatches():
    ...
    if matches1 != 0 or matches2 != 0 or wins1 != 0 or wins2 != 0:
        raise ValueError(
            "Newly registered players should have no matches or wins. %s%s%s%s" %(matches1,matches2, wins1,wins2))
...
def testReportMatches():
    ...
    for (i, n, w, m) in standings:
        if m != 1:
            raise ValueError("Each player should have one match recorded.%s"%m)
